...
import org.mrpdaemon.sec.comp.Compatibility;
...
class VolumeKey {
	...
	private static byte[] encryptVolumeKey(EncFSConfig config,
		...
		byte[] passKeyData = Compatibility.copyOfRange(pbkdf2Data, 0, keySizeInBytes);
		byte[] passIvData = Compatibility.copyOfRange(pbkdf2Data, keySizeInBytes,
	...
	protected static byte[] decryptVolumeKey(EncFSConfig config,
		...
		//System.out.println("PBKDF2 Data " + Hex.encodeHexString(pbkdf2Data));
		...
		//System.out.println("cipherVolKeyData " + Hex.encodeHexString(cipherVolKeyData));
		
		byte[] encryptedVolKey = Compatibility.copyOfRange(cipherVolKeyData, 4,
				cipherVolKeyData.length);
		//System.out.println("encryptedVolKey " + Hex.encodeHexString(encryptedVolKey));
		
		// Prepare key/IV for decryption
		int keySizeInBytes = config.getVolumeKeySizeInBits() / 8;
		byte[] passKeyData = Compatibility.copyOfRange(pbkdf2Data, 0, keySizeInBytes);
		//System.out.println("PassKeyData " + Hex.encodeHexString(passKeyData));
		
		byte[] passIvData = Compatibility.copyOfRange(pbkdf2Data, keySizeInBytes,
				keySizeInBytes + EncFSVolume.IV_LENGTH_IN_BYTES);
		//System.out.println("PassIvData " + Hex.encodeHexString(passIvData));

		Key passKey = EncFSCrypto.newKey(passKeyData);
		byte[] ivSeed = Compatibility.copyOfRange(cipherVolKeyData, 0, 4);
		//System.out.println("ivSeed " + Hex.encodeHexString(ivSeed));
		...
		//System.out.println(clearVolKeyData);
