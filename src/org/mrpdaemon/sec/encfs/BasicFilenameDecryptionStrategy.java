...
abstract class BasicFilenameDecryptionStrategy extends
	...
	protected String decryptImpl(String fileName)
		...
		byte[] chainIv;
		try {
			chainIv = EncFSCrypto.computeChainedIV(volume, volumePath,
					config);
		} catch (UnsupportedEncodingException e2) {
			chainIv = new byte[1];
			e2.printStackTrace();
		}
		byte[] base256FileName;;
		base256FileName = EncFSBase64.decodeEncfs(fileName.getBytes());
		byte[] macBytes;
		...
		byte[] encFileName = Compatibility.copyOfRange(base256FileName, 2,
	...
	private void verifyDecryptionWorked(EncFSVolume volume, byte[] chainIv,
		...
		byte[] expectedMac = Compatibility.copyOfRange(base256FileName, 0, 2);
